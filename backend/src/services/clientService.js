...
export const clientService = {
  ...
  async getClientRoleId() {
    const { role } = await RoleService.getRoleByCode(ROLES.CLIENT);
    return role.id;
  },
  ...
  async getClientById(userId) {
    ...
    if (result.data.role_id !== roleId) {
      return {
        success: false,
        message: "El usuario existe pero no tiene rol de cliente.",
      };
    }
  ...
  async updateClient(userId, payload) {
    ...
    if (existing.data.role_id !== roleId) {
      return {
        success: false,
        message: "El usuario existe pero no es cliente.",
      };
    }
  ...
  async deactivateClient(userId) {
    ...
    if (error) {
      throw new DatabaseError("Error desactivando al cliente.", error.message);
    }
