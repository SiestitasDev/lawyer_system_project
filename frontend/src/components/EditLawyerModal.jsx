...
import { updateLawyer } from "../services/admin/adminService"

export const EditLawyerModal = ({ isOpen, onClose, onSuccess, lawyer }) => {
  ...
  const fetchLawyerData = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:4000/api/admin/lawyers/${lawyer.id}`, {
        method: 'GET',
        headers: {
      ...
      const lawyerData = data.data[0]
      setFormData({
        name: lawyerData.name || '',
        email: lawyerData.email || '',
        is_active: lawyerData.is_active || false
      })
      setError('')
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateLawyer(lawyer.id, lawyerData, token)
      
      onSuccess()
      onClose()
    } catch (error) {
      setError(error.message || 'Error al actualizar el abogado')
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div className="flex justify-between items-center p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-2xl font-bold text-gray-900">Editar Abogado</h2>
          <button 
            onClick={onClose}
